...
import { MessageSquare, FileText, Brain, Trophy, Settings, LogOut, Sun, Moon } from 'lucide-react'
...
export default function Sidebar({ activePanel, setActivePanel }: SidebarProps) {
  const { theme, toggleTheme, signOut, activePdf, setIsSettingsOpen } = useStore()
  ...
  return (
    <div className="w-64 h-full border-r border-[var(--glass-border)] flex flex-col shrink-0" style={{ backgroundColor: 'var(--bg-sidebar)' }}>
      <div className="h-20 flex items-center px-6 gap-3 cursor-pointer" onClick={() => navigate('/dashboard')}>
