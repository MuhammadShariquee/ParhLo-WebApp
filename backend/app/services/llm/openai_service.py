...
class OpenAIService:
    def __init__(self):
        ...
        if self.api_key:
            ...
            logger.info("OpenAI initialized for routing.")
        else:
            self.client = None
            logger.warning("OPENAI_API_KEY not set for OpenAIService.")

    async def generate_response(self, prompt: str, mode: str = "chat", system_prompt: str = None) -> str:
        if not self.client:
            raise Exception("API key error: OpenAI API key not configured")
        ...
        try:
            ...
            if "429" in error_str:
                logger.warning(f"OpenAI 429 Too Many Requests: {e}")
                raise Exception("429 Too Many Requests") from e
            elif "503" in error_str:
                logger.warning(f"OpenAI 503 Service Unavailable: {e}")
